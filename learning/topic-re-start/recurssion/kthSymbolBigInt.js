...
function kthSymbol(A, B) {
    ...
    for (let i = A; i > 0; i--) {
        const length = BigInt(1) << BigInt(i - 1); // Equivalent to 2^(i-1)

        if (index >= length) {
            result = result ^ 1; // Flip the result
            index = index - length; // Move to the corresponding index in the previous row
        }
    }
