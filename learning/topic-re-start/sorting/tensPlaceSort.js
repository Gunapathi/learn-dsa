...
function solve(A) {
    return A.sort((a, b) => {
        const tensA = (BigInt(a) / BigInt(10)) % BigInt(10);
        const tensB = (BigInt(b) / BigInt(10)) % BigInt(10);

        if (tensA === tensB) {
            return Number(BigInt(b) - BigInt(a));
        } else {
            return Number(tensA - tensB);
        }
    });
}

console.log(solve([6, 17, 15, 13, 112])) // [7, 112, 19, 15, 11]
